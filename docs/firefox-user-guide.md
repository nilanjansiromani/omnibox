# SuperOmniBar - Firefox User Guide

## Welcome to SuperOmniBar for Firefox!

SuperOmniBar brings the same powerful search experience to Firefox users. With a beautiful, liquid glass-styled interface, you can instantly search through your tabs, bookmarks, and browsing history using a simple keyboard shortcut.
...
3. **Verify Installation**
   - You should see SuperOmniBar in your temporary add-ons list
   - The extension is now active

**Important Note**: Temporary add-ons in Firefox are removed when you restart Firefox. To make it permanent, you'll need to:
- Use Firefox Developer Edition with permanent installation options, or
- Sign and publish the extension through Firefox Add-ons
...
SuperOmniBar uses Firefox's `browser` API with Chrome fallback for maximum compatibility. This ensures:
- Works with Firefox's security model
- Handles Firefox-specific URLs (`about:`, `moz-extension://`)
- Compatible with Firefox 109+ (Manifest V3)

### Temporary Add-on Behavior

**Important**: The extension loads as a temporary add-on, which means:
- It's removed when Firefox restarts
- You'll need to reload it after restarting Firefox
- Settings and data are preserved during the session
...
2. Verify keyboard shortcut: Go to `about:addons` → Gear icon → Manage Extension Shortcuts
...
**Solution**: This is normal for temporary add-ons. Reload it:
1. Go to `about:debugging`
2. Click "This Firefox"
3. Click "Load Temporary Add-on..."
4. Select `manifest.json` from the `firefox/` folder
...
**Firefox Security**: Firefox's security model ensures the extension can only access what you've granted permission for.

---

## 💡 Best Practices

1. **Keep It Loaded**: Reload after Firefox restarts for quick access
...
You're now ready to navigate Firefox faster than ever. The more you use it, the more natural it becomes. Happy browsing!
